...
const executeScript = async () => {
    ...
    const baseURL = process.env.STAGING_BASE_URL; //APIのホストURL
    const outputDir = process.env.OUTPUT_DIR; //完成ファイルの転送先
    const headers = { 'x-api-key': process.env.STAGING_API_KEY }; //headersに必要な変数(U members IDはoptionalだったため省略)
            ...
            try {
                await Promise.all(filePaths.map(async (path) => {
                    ...
                    if (fs.existsSync(destPath)) {
                        await fs.promises.unlink(destPath); // 既存のファイルを削除
                    }
            
                    await fs.promises.rename(path, destPath); // 新しいファイルを移動
                    console.log(`File moved: ${path} -> ${destPath}`);
                }));
